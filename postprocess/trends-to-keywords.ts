...
const yesterdayMorning = yesterday.setHours(0, 0, 0); // after midnight
const yesterdayMidnight = yesterday.setHours(23, 59, 59); // before midnight
const trendingInLast24Hour = trends.filter(({timestamp}) => {const current = new Date(timestamp);
...
const currentTrends = [...new Map(trendingInLast24Hour.map(item =>
